  ...
  return (
    <section className="relative px-5 py-24 sm:px-6 lg:px-8 lg:py-32">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="relative mt-16 grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          <div className="absolute left-8 right-8 top-16 hidden h-px bg-gradient-to-r from-transparent via-[#d6b46a]/60 to-transparent xl:block" />

          {steps.map(({ title, icon: Icon, text }, index) => (
            <article
              key={title}
              className="relative rounded-[1.6rem] border border-white/75 bg-white/72 p-6 shadow-[0_18px_55px_rgba(31,41,55,0.08),inset_0_1px_0_rgba(255,255,255,0.95)] backdrop-blur-xl transition duration-300 hover:-translate-y-2 hover:border-[#d6b46a]/45 hover:shadow-[0_28px_80px_rgba(31,41,55,0.13)]"
            >
              <div className="mb-8 flex items-center justify-between">
                <div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-[#111] text-[#d6b46a] shadow-[0_18px_40px_rgba(0,0,0,0.16)]">
                  <Icon size={22} strokeWidth={1.8} />
                </div>
                <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#9b7a3e]">
                  0{index + 1}
                </span>
              </div>
